...
export default class Single extends React.Component {
    ...
    componentWillMount() {
        let props = this.props;
        let state = props.location.state;
        FetchData('type=song&id=' + state.id + '&br=320000', 'get').then(res => {
            res.json().then(response => {
                this.setState({
        ...
        // 获取歌词
        FetchData('type=lyric&search_type=1006&id=' + state.id, 'get').then(res => {
            res.json().then(response => {
                if (response.nolyric) {
                    this.setState({
                        lyric: [
                ...
                <div>
                    <div className='single_music'>
                        ...
                        <div className='container' ref='container'>
                            <i className='material-icons volume' style={{ zIndex: this.state.lyricShow ? 1 : 0 }}>
                                volume_up
                            </i>
                            <div
                                className='volume_bar'
                                onClick={event => this.volume(event)}
                                style={{ zIndex: this.state.lyricShow ? 1 : 0 }}
                            >
                            ...
                            <div className='lyric' style={{ zIndex: this.state.lyricShow ? 1 : 0 }}>
                                <div
                                    className='lyric_show'
                                    style={{ top: this.state.top + 'px' }}
                                    onClick={() => this.lyricShow()}
                                    ref='lyric'
                                >
                                    {this.state.lyric.map((item, index) => {
                                        return (
                                            <p
                                                key={index}
                                                data-time={item.time.slice(1, 6)}
                                                className={index === this.state.cur ? 'line_lyric show' : 'line_lyric'}
                                            >
                                                {item.lyric}
                                            </p>
                                        );
                            ...
                            <div
                                className='bg_animation'
                                style={{ zIndex: this.state.lyricShow ? 0 : 1 }}
                                onClick={() => this.lyricShow()}
                            >
                                <img
                                    src={this.props.location.state.picUrl}
                                    alt=''
                                    style={{ animationPlayState: playState }}
                                />
                                <div
                                    className='download_dis_icon'
                                    style={{ display: this.state.lyricShow ? 'none' : 'block' }}
                                >
                                    <i
                                        className='material-icons'
                                        style={{ display: this.state.favorite ? 'inline-block' : 'none' }}
                                        onClick={event => this.favorite(event)}
                                    >
                                        favorite
                                    </i>
                                    <i
                                        className='material-icons'
                                        style={{ display: this.state.favorite ? 'none' : 'inline-block' }}
                                        onClick={event => this.favorite(event)}
                                    >
                                        favorite_border
                                    </i>
                                    <i className='material-icons' onClick={event => this.download(event)}>
                                        arrow_downward<a ref='download'></a>
                                    </i>
                                    <span className='comment'>
                                        <i className='material-icons' onClick={event => this.comment(event)}>
                                            message
                                        </i>
                                        <span className='total'>
                                            {this.state.commentTotal > 999
                                                ? '999+'
                                                : this.state.commentTotal > 99
                                                ? '99+'
                        ...
                        <footer className='progress_bar'>
                            ...
                            <div className='rate' ref='pro_bar_w' onClick={event => this.timePlay(event)}>
                                <div
                                    className='buffer'
                                    style={{
                                        width: (this.state.buffer / this.state.duration) * this.state.barWidth + 'px'
                                    }}
                                >
                                ...
                                <div
                                    className='played'
                                    style={{
                                        width: (this.state.played / this.state.duration) * this.state.barWidth + 'px'
                                    }}
                                ></div>
                            </div>
                            ...
                            <audio
                                ...
                                ref='audio'
                                onTimeUpdate={event => this.progress(event)}
                            ></audio>
                            <div className='button_list'>
                                ...
                                <div className='play_pause' onClick={() => this.playStatus()}>
                                    ...
                                    <i
                                        className='material-icons'
                                        style={{ display: this.state.play ? 'none' : 'block' }}
                                    >
                                        play_arrow
                                    </i>
                                </div>
                    ...
                    <div className='comments_show' style={{ display: this.state.commentShow ? 'block' : 'none' }}>
                        <Comment
                            ...
                            commentState={this.commentStatus.bind(this)}
                            id={this.props.location.state.id}
                            bgImg={this.props.location.state.picUrl}
                        />
                    </div>
                </div>
            );
